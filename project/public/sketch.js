...
function setFxParamsSettings() {
  ...
  if (scale_selection === "CUSTOM") {
    ...
    fx_paramsArray = ["CUSTOM", custom_width, custom_height, [16, 22]];
  } else {
  ...
  var paperIndex = canvas_size_storage.findIndex(
    (item) => item[0] === selectedPaperSize,
  );
  resolution_data =
    paperIndex !== -1
      ? canvas_size_storage[paperIndex]
      : canvas_size_storage[default_size_id];
...
function set_lines_colors_random_orel() {
  ...
  var color_sequence =
    "771112226664445554446662221131133113373311331131122266664444555444466662222"; //
  //  ->|
...
function set_piece_name() {
  piecename =
    ...
    (lines_per_tiles - 1) +
    "l-" +
    pen_size[default_pen_id][0] +
    "-ls" +
    Math.floor(lines_space);
  filename = piecename;
...
function draw_layer(layer) {
  ...
  var pix = drawings[0];
  var vect = drawings[1];
  var drawn_canvas_height =
    ((vertical_tiles - 1) * cells_diag) / 2 + cells_diag;
  ...
  for (var x = 0; x < horizontal_tiles; x++) {
    for (var y = 1; y <= Math.ceil(vertical_tiles); y++) {
      ...
      if (debug_mode_activated) {
        ...
        local_vector_canvas.push();
        if (y % 2 == 0) {
          local_vector_canvas.translate(
            x * cells_diag + cells_diag / 2,
            cells_diag / 2 + ((y - 1) * cells_diag) / 2,
          );
        } else {
          local_vector_canvas.translate(
            x * cells_diag + cells_diag,
            cells_diag / 2 + ((y - 1) * cells_diag) / 2,
          );
        }
        local_vector_canvas.rotate(radians(45));
      ...
      if (debug_mode_pattern_activated)
        local_vector_canvas.image(
          tile_debug_view,
          0,
          0,
...
document.addEventListener("DOMContentLoaded", () => {
  document
    ...
    .addEventListener("click", updateSingleParam);
});
...
function updateParameter(key, value) {
  ...
  if (finalValue !== undefined) {
    ...
    if (paramDef) {
      ...
      console.log("Fx params updated by UI");
...
function isClickInsideRotatedSquare(
  mouseX,
  mouseY,
  centerX,
  centerY,
...
function check_tile_under_mouse(event) {
  ...
  for (var x = 0; x < horizontal_tiles; x++) {
    for (var y = 0; y <= vertical_tiles; y++) {
      ...
      click = isClickInsideRotatedSquare(
        event.x / scaled_ratio,
        event.y / scaled_ratio,
        cell.x_position,
        cell.y_position,
        local_cell_size,
        45,
...
function setDimensions() {
  ...
  var windowScale_Width = map(
    -10 + html_Canvas_Size / (canvas_Height / canvas_Width),
    0,
    canvas_Width,
    0,
    1,
